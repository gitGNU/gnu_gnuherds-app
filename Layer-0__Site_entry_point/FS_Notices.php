...
require_once "../Layer-2__Business_logic/content/forms/FS_Notices_form.php";

if ( $_GET['format'] == 'rss' ) //XXX: Note that RSS is not supported in general listing, at least yet.
{
	// If the requested format is RSS we do not apply the web page template.
	$fsNoticesForm = new FSNoticesForm();
	$fsNoticesForm->printOutput();
}
else
{
	$fsNoticesForm = new FSNoticesForm();

	$webPage = new WebPage($fsNoticesForm);
	$webPage->processPage();
	$webPage->printPage();
}
?>
