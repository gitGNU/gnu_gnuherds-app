...
{t}The GNU Herds' official voting mechanism is not intented to:{/t}
	<ul>
		<li>{t}be used as a country voting system.{/t}</li>
		<li>{t}be used as the FS community voting system. The FS community does not need a voting system. As Barry Fitzgerald exposes:{/t} <i>"Free Software embodies democracy in an anarchistic, classical democracy kind of way. The freedom of the community is itself the force of democracy"</i>.</li>
	</ul>
</p>
<p>
{t}The GNU Herds' official voting mechanism is only intented to:{/t}
	<ul>
		<li>{t}be used as voting system to this Association. The voting mechanism must be used as a tool to keep GNU Herds moving in the right direction, helping members guide the board members. It can also be used to get opinions about ideas, etc.: binding vote, opinion vote.{/t}</li>
	</ul>
</p>
...
{t}There are several projects which try to solve the e-Voting trouble. The goal of each project can be different. Some of them has modified its aims or are stalled:{/t}
	<ul>
		<li><a href="http://www.gnu.org/software/free/" target="_top">GNU.FREE</a>: {t}The development has been discontinued due to{/t} <i>"creating an Internet Voting system sufficiently secure, reliable and anonymous is extremely difficult, if not impossible"</i>.</li>
		<li><a href="http://glasnost.entrouvert.org" target="_top">Glasnot</a>. {t escape='no'
		  1='<i>'
		  2='</i>'
		}It seems to support the %1Condorcet method%2 and even "a mix of secret and public ballots".{/t}</li>
...
<ol>
	<li>[Central server] &nbsp; <a href="http://www.gnu.org/software/free/" target="_top">GNU.FREE</a> &nbsp; <b>(State:&nbsp;REJECTED&nbsp;?&nbsp;)</b>
		<p><i>Disadvantage</i>: Central server. Jose's opinion:
...
<ul>
    <li><a href="http://www.thebell.net/papers/vote-req.pdf" target="_top">Voting System Requirements</a>. {t}A 16 page nontechnical paper, on 16 requirements that a voting system "must" meet. Read it to know a bit more about efforts underway at coming up with secure voting (and what may constitute a definition of secure voting). Consider in particular the requirement that anonimity be maintained.{/t}</li>
    <li><a href="http://www.gnu.org/software/free/" target="_top">GNU.FREE links</a>. {t}We want remark:{/t}<br />
