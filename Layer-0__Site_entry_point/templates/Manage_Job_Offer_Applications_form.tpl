...
{if count($entityId) == 0 }
	<p>&nbsp;</p>
	<p>{t}There are not any application subscribed to this job offer{/t}.</p>
	<p>&nbsp;</p>
{else}
...
<td class="{if $i % 2}tdDark{else}tdLight{/if}"><strong>{t}{$countryName[$i]}{/t}</strong>{if $stateProvince[$i]}, {$stateProvince[$i]}{/if}{if $city[$i]}, {$city[$i]}{/if}</td>
<td class="{if $i % 2}tdDark{else}tdLight{/if}">{$professionalExperienceSinceYear[$i]}</td>
<td class="{if $i % 2}tdDark{else}tdLight{/if}">
