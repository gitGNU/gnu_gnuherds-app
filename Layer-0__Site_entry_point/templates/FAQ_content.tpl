...
<p>{t}Index{/t}</p>
<ol>
<li><a href="#Why_another_Job_Site?" style="text-decoration: none;" target="_top">{t}Why another Job Site?{/t}</a></li>
<li><a href="#How_to_subscribe_to_a_job_offer?" style="text-decoration: none;" target="_top">{t}How to subscribe to a job offer?{/t}</a></li>
<li><a href="#Who_can_subscribe_to_a_job_offer?" style="text-decoration: none;" target="_top">{t}Who can subscribe to a job offer?{/t}</a></li>
<li><a href="#Who_can_post_a_job_offer?" style="text-decoration: none;" target="_top">{t}Who can post a job offer?{/t}</a></li>
<li><a href="#How_is_the_Job_Site_module_related_to_the_GNU_Herds_Association?" style="text-decoration: none;" target="_top">{t}How is the Job Site module related to the GNU Herds Association?{/t}</a></li>
</ol>
<p>&nbsp;</p>
<p>&nbsp;</p>
<h4>1. {t}Why another Job Site?{/t} <a name="Why_another_Job_Site?"></a></h4>
	<p>{t}Because no one comply the below proposed conditions:{/t}</p>

	<ul>
		<li> {t escape='no'
		  1='<a href="http://www.gnu.org/licenses/agpl.html" target="_top">'
		  2='</a>'
		}Use a license to cover use of the software over a computer network. The current %1Affero GPL%2 license will be replaced by the GPL v3 license when it be released. Anyhow, the project is always open to discuss this and any other subject.{/t}</li>
		<li> {t escape='no'
		...
		}Moreover, the management of the jobsite must be %1controlled by its own users%2 using the democratic policy. That will be guaranteed by the GNU Herds association policy. So, being sure the evolution of the jobsite will be controlled and led by its users.{/t}</li>
		<li> {t}Supplying support to engage among companies and persons, it doesn't matter the which or the order, helping the raise of a diversity of business models.  That is flexibility.{/t}</li>
		<li> {t}Make it easy the contacts among persons and companies interested in Free Software business.{/t}</li>
		<li> {t}Having a global scope, supporting business models based on long-distance jobs. With future technologies the localisation must not be an obstacle to establish relationships about IT services. So, the goal, already got, to translate the webapp to other languages, and to support the fact that a job offer could be inserted in several languages.{/t}</li>
	</ul>
	<p>
	{t escape='no'
	  1='<a href="/Charter.php#Aims" target="_top">'
	  2='</a>'
...
<h4>2. {t}How to subscribe to a job offer?{/t}<a name="How_to_subscribe_to_a_job_offer?"></a> </h4>
	<ul>
		<li> {t escape='no'
		  ...
		  4='</a>'
		}Register as new %1Person%4, %2Company%4 or %3non-profit Organization%4.{/t}</li>
		<li> {t escape='no'
		  1='<a href="/Qualifications.php">'
		  2='</a>'
		}Fill the %1Qualifications%2 form.{/t}</li>
		<li> {t}Log in.{/t}</li>
		<li> {t}Subscribe to the offer.{/t}</li>
	</ul>
	<p>{t}Note that some offers could allow to subscribe only to some entity types: Persons, Companies or non-profit Organizations.{/t}</p>
<div align="center"><a href="#" style='text-decoration: none;' target="_top">{t}Back to top{/t}</a></div>
<p>&nbsp;</p>
...
<h4>5. {t}How is the Job Site module related to the GNU Herds Association?{/t}<a name="How_is_the_Job_Site_module_related_to_the_GNU_Herds_Association?"></a></h4>
	<p>{t escape='no'
	  ...
	  2='</a>'
	}The Job Site module is %1controlled%2 by the Association.{/t}</p>
	<p>&nbsp;</p>
