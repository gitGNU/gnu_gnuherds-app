...
class Donation
{
	...
	{
		$sqlQuery = "PREPARE query(integer) AS  SELECT D1_Donation,E1_Email,EP_FirstName,EP_LastName,EP_MiddleName,EC_CompanyName,EO_OrganizationName FROM D1_Donations2JobOffers,E1_Entities WHERE D1_E1_Id=E1_Id AND D1_J1_Id=$1 ;  EXECUTE query('$JobOfferId');";
	...
	public function addDonation($JobOfferId)
	{
		...
		// If the user is logged in the donation is already confirmed; else, we create confirmation fields
		...
		else
		{
			...
			$entity = new Entity();
			$EntityId = $entity->getEntityId(trim($_POST['Email']),'REQUEST_TO_ADD_DONATION',$magic); // It registers the email or send the verification message if the address is already registered
